...
int main()
{
    ...
    for (int i = 0; i < 11; i++)
    {
        free_list[i] = (priQ *)malloc(sizeof(priQ));
        priQcreate(free_list[i], 50);
    }
    ...
    p.runtime = 2;
    p.remainingtime = 2;
    p.priority = 0; // the starting address of the hole
    p.memsize = 256;
    p.memStart = 0;

    // look for a suitable memory unit for the process
    // initially, there is only one hole
